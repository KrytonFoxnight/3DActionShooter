...
> ## ✅ 종료 (2026-08-30)
>
> **이 문서는 진행 로그입니다. 계획 문서로는 더 이상 유효하지 않습니다.**
> 아래 사다리는 2026-08-25 시점의 계획 원본이며, 대조를 위해 고치지 않고 그대로 둡니다.
> 실제 결과와 계획이 갈린 지점은 바로 아래 "실제 결과"에 적습니다.
>
> ### 실제 결과
>
> | 사다리 | 계획 | 실제 |
> |---|---|---|
> | 0. 사망 처리 | 구현 | **완료.** `PlayerCharacter.cs:126`, `EnemyCharacter.cs:129` |
...
> ### 안 한 것 (판단에 따른 제외)
>
> 보상 선택 / 결과 화면 / 원거리 적 / 락온 / Object Pooling / VFX / 피격 무적 시간 / 부채꼴 각도 판정.
> 무적은 디버그용 플래그만 있습니다 (`PlayerCharacter.cs:23`). i-frame 규칙은 구현하지 않았습니다.
...
→ 설명 포인트가 "직접 짰다" → **"파라미터 검증하고 조정했다"**로 옮겨감
...
- [ ] dev → main PR 병합
- [ ] ~~`planning-guideline.md` 맨 위에 동결 한 줄 추가~~ → 해당 문서는 2026-08-30에 삭제
...
```
08-30~       저장소 정리 → main 병합 → README → GIF
```
