...
**때리는 쪽**의 타이밍을 다룹니다. **맞는 쪽**(피격 리액션·경직)을 단독으로 다루는 문서는 아직 없습니다.
경직의 소유권과 `Damaged`/`Died` 분리는 [유닛 상태 권한](./unit-state-authority.md)에서 일부 다룹니다.
...
클립의 임팩트 프레임에 이벤트를 심어 무기 콜라이더를 Toggle. 애니메이션과 데미지 처리의 동기화가 구조적으로 보장되도록 함.
...
**장점**: 모션과 판정이 근본적으로 어긋날 수 없음. 클립을 교체 시 자동으로 판정이 따라옴. 애니메이터가 전투 감각을 소유.
**단점**: 판정 로직이 클립 자산에 종속됨. 서버는 Animator를 돌리지 않으므로 서버 권위 구조로 가기 어려움.
...
대기 중인 판정은 확정된 것이 아니라 **취소될 수 있음**. 피격 시 대기 중인 판정을 끊으며, 끊을지 여부도 데이터(`interruptibleByHit` — `false`면 슈퍼아머)로 정함. 상세는 후속 문서 참조.
...
- 레퍼런스 프로젝트
  ...
  - 인터럽트 정책도 데이터임 — `ActionConfig`의 `ActionInterruptible` / `IsInterruptableBy` / `BlockingMode`

**장점**: 서버가 Animator 없이 판정 데이터(타이밍·수치)만으로 시뮬레이션 가능. 밸런싱 작업이 코드 수정 없는 콘텐츠 작업으로 가능해짐. Animation과 무관하게 로직 테스트 가능.
...
- PvP급 기법(서버 애니메이션 리와인드, 결정론+롤백, 클립→데이터 베이크)들을 조사한 결과, **어느 계보든 서버가 판정 시점에 쓰는 것은 결국 "데이터화된 애니메이션"이었음**.
- ①을 유지하는 대규모 프로젝트도 네트워크 기반 콘텐츠 개발 시 ②번 방식과 유사해지는 경향 (베이크 툴체인으로 자동 변환). 그래서 처음부터 ②로 시작하고자 했음.

### 이유 2: 1인 개발 + 무료 에셋 환경에서 ①의 장점이 유의미하지 않음

- ①의 최대 장점 "애니메이터가 전투 감각을 소유"는 전담 애니메이터가 있을 때 유지보수 관점에서 유리함. 프로그래머 1인 개발에는 해당 사항 없음.
...
- 데미지·사거리·판정 시점·경직·인터럽트 여부 등 **게임플레이 판정은 데이터**로 둠 (`hitDelay`, `hitReactionCooldown`, `hitStunDuration`, `interruptibleByHit`)
- 검격 이펙트·사운드·발소리 등 **어긋나도 게임이 안 깨지는 표현은 Animation Event**로 분담
